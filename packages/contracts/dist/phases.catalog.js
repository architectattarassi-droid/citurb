...
exports.BORNES = exports.LOTS_SECOND_OEUVRE = exports.TEMPLATE_PLANCHER = exports.TEMPLATE_SOUS_SOL = exports.PHASES_FIXES = void 0;
exports.generatePhases = generatePhases;
exports.buildPhaseIndex = buildPhaseIndex;
exports.validateDossier = validateDossier;
...
function planchersHorsSol(nbEtagesHorsSol) {
    const out = [{ title: 'RDC', slugPart: 'rdc' }];
    for (let i = 1; i <= nbEtagesHorsSol; i++)
        out.push({ title: `R+${i}`, slugPart: `r${i}` });
 ...
 */
function _generatePhasesInternal(dossier) {
    const fixes = exports.PHASES_FIXES.filter((p) => {
        if (p.conditionFlag && !dossier.flags[p.conditionFlag])
            return false;
        if (p.conditionFoncier && !p.conditionFoncier.includes(dossier.typeOperationFoncier))
            return false;
        return true;
    ...
    for (const p of fixes) {
        ...
        if (p.code === '84_FONDATIONS') {
            ...
            for (let n = dossier.nbSousSols; n >= 1; n--) {
                ...
                out.push({ ...exports.TEMPLATE_SOUS_SOL, code, slug: `chantier-sous-sol-${n}`, title: `Sous-sol −${n}`,
            ...
            niveaux.forEach((niv, i) => {
                ...
                out.push({ ...exports.TEMPLATE_PLANCHER, code, slug: `chantier-gros-oeuvre-${niv.slugPart}`, title: `Plancher ${niv.title}`,
        ...
        if (p.code === '90_RECEPTION_GROS_OEUVRE') {
            const applicables = dossier.lotsApplicables
                ? exports.LOTS_SECOND_OEUVRE.filter((l) => dossier.lotsApplicables.includes(l.slug))
                : exports.LOTS_SECOND_OEUVRE;
            ...
            applicables.filter((l) => l.vague === 'RESEAUX' || l.vague === 'INDEPENDANT').forEach((lot, idx) => {
                ...
                if (lot.vague === 'RESEAUX')
                    lotsReseaux.push(lot.code);
            });
    ...
    if (auto) {
        ...
        if ((dossier.typeOperationFoncier !== 'aucun') && out.some((x) => x.code === '29_AUTORISATION_LOTIR'))
            pre.add('29_AUTORISATION_LOTIR');
        if (dossier.flags.securiteIncendie && out.some((x) => x.code === '24_AVIS_PROTECTION_CIVILE'))
            pre.add('24_AVIS_PROTECTION_CIVILE');
        auto.prerequis = [...pre];
    }
    ...
    if (conf) {
        ...
        if (dossier.flags.securiteIncendie && out.some((x) => x.code === '95A_CONFORMITE_SECURITE_INCENDIE'))
            pre.add('95A_CONFORMITE_SECURITE_INCENDIE');
        if (dossier.flags.ascenseur && out.some((x) => x.code === '95C_CONFORMITE_ASCENSEUR'))
            pre.add('95C_CONFORMITE_ASCENSEUR');
        if (dossier.flags.raccordementsDefinitifs && out.some((x) => x.code === '95B_RACCORDEMENTS_DEFINITIFS'))
            pre.add('95B_RACCORDEMENTS_DEFINITIFS');
        conf.prerequis = [...pre];
    }
    ...
    if (dce) {
        if (out.some((x) => x.code === '48_METRE_QUANTITATIF'))
            dce.prerequis = ['48_METRE_QUANTITATIF'];
        else if (out.some((x) => x.code === '42_VISA_STRUCTURE_CONTROLE'))
            dce.prerequis = ['42_VISA_STRUCTURE_CONTROLE'];
    }
    ...
    if (dossier.flags.financementBancaire) {
        ...
        if (racc && out.some((x) => x.code === '57_DEBLOCAGE_INITIAL'))
            racc.prerequis = [...new Set([...racc.prerequis, '57_DEBLOCAGE_INITIAL'])];
    }
    ...
    if ((dossier.typeOperationFoncier !== 'aucun')) {
        ...
        if (auto2 && out.some((x) => x.code === '29D_BORNAGE_TITRES'))
            auto2.prerequis = [...new Set([...auto2.prerequis, '29D_BORNAGE_TITRES'])];
    }
    ...
    if (dossier.flags.securiteIncendie) {
        ...
        if (confInc && out.some((x) => x.code === 'LOT_SECURITE_INCENDIE'))
            confInc.prerequis = [...new Set([...confInc.prerequis, 'LOT_SECURITE_INCENDIE'])];
    }
    ...
    if (dossier.flags.chantierComplexe) {
        ...
        if (ouv) {
            const adds = [];
            if (out.some((x) => x.code === '55_MANDAT_OPC'))
                adds.push('55_MANDAT_OPC');
            if (out.some((x) => x.code === '56B_MISSION_SPS'))
                adds.push('56B_MISSION_SPS');
            if (adds.length > 0)
                ouv.prerequis = [...new Set([...ouv.prerequis, ...adds])];
        }
    }
    return out.sort((a, b) => a.order - b.order);
}
function buildLot(lot, order, prerequis) {
    return {
 ...
 */
function generatePhases(dossier) {
    const v = validateDossier(dossier);
    if (!v.ok)
        throw new Error(`Dossier invalide, génération refusée :\n- ${v.errors.join('\n- ')}`);
 ...
 */
function buildPhaseIndex(dossier) {
    const v = validateDossier(dossier);
    if (!v.ok)
        throw new Error(`Dossier invalide, génération refusée :\n- ${v.errors.join('\n- ')}`);
...
const PORTES_VALIDES = ['P1', 'P2'];
const FONCIER_VALIDES = ['aucun', 'morcellement', 'groupeHabitation', 'lotissement'];
const FLAGS_REQUIS = [
...
function validateDossier(dossier) {
    ...
    const d = dossier;
    if (!PORTES_VALIDES.includes(d.porte))
        return { ok: false, errors: ['porte invalide : attendu P1 ou P2.'] };
    if (!FONCIER_VALIDES.includes(d.typeOperationFoncier))
    ...
    for (const key of FLAGS_REQUIS) {
        if (typeof flags[key] !== 'boolean')
            return { ok: false, errors: [`flags.${key} doit être un booléen.`] };
    }
    ...
    const errors = [];
    if (dossier.nbEtagesHorsSol < 0)
        errors.push('nbEtagesHorsSol doit être >= 0 (0 = RDC seul).');
    if (dossier.nbSousSols < 0)
        errors.push('nbSousSols ne peut être négatif.');
    const b = exports.BORNES[dossier.porte];
    if (dossier.nbEtagesHorsSol > b.maxEtagesHorsSol)
        errors.push(`nbEtagesHorsSol dépasse la borne ${dossier.porte} (max ${b.maxEtagesHorsSol}).`);
    if (dossier.nbSousSols > b.maxSousSols)
        errors.push(`nbSousSols dépasse la borne ${dossier.porte} (max ${b.maxSousSols}).`);
    ...
    if (dossier.lotsApplicables) {
        const valides = new Set(exports.LOTS_SECOND_OEUVRE.map((l) => l.slug));
        for (const s of dossier.lotsApplicables)
            if (!valides.has(s))
                errors.push(`Lot inconnu : ${s}`);
    }
    ...
    for (const p of ordered) {
        if (seenSlug.has(p.slug))
            errors.push(`Collision slug : ${p.slug}`);
        if (seenCode.has(p.code))
            errors.push(`Collision code : ${p.code}`);
        if (seenOrder.has(p.order))
            errors.push(`Collision order : ${p.order} (${p.code})`);
        if (p.slug.includes('/'))
            errors.push(`Slug non plat : ${p.slug}`);
        seenSlug.add(p.slug);
        seenCode.add(p.code);
        seenOrder.add(p.order);
        for (const pre of p.prerequis)
            if (!codes.has(pre))
                errors.push(`Prérequis inexistant : ${p.code} → ${pre}`);
    }
    ...
    for (const p of ordered)
        for (const pre of p.prerequis)
            if (codes.has(pre)) {
                adj.get(pre).push(p.code);
                indeg.set(p.code, indeg.get(p.code) + 1);
            }
    ...
    if (visited !== ordered.length)
        errors.push('Cycle détecté dans le graphe de prérequis.');
    ...
    const aDesDependants = new Set();
    for (const p of ordered)
        for (const pre of p.prerequis)
            aDesDependants.add(pre);
    for (const p of ordered) {
