...
import type { Lead } from "./lead-funnel.types";
...
export function isValidMaPhone(t: string | undefined): boolean {
  if (!t) return false;
  return /^(\+212|0)[567]\d{8}$/.test(t.replace(/[\s\-]/g, ""));
}

export function isValidEmail(e: string | undefined): boolean {
  if (!e) return false;
...
export interface ScoreInput {
  ...
  wizardStep?: number;
  respondedJ1WithinHours?: number; // < 24 → +20
}
...
export function computeLeadScore(input: ScoreInput): ScoreOutput {
  ...
  if (input.returnVisitor === true) b.return = 10;
  ...
  if (
    typeof input.respondedJ1WithinHours === "number" &&
  ...
  const score = Math.min(
    100,
    Math.max(0, Object.values(b).reduce((a, v) => a + v, 0)),
  );
...
export function rescoreLead(lead: Lead): ScoreOutput {
...
function detectJ1Response(lead: Lead): number | undefined {
