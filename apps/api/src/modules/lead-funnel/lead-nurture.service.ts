...
interface Template {
  ...
  body: (lead: Lead) => Record<Lang, string>;
}

const PUBLIC_URL = () =>
  process.env.PUBLIC_WEB_URL || "https://citurbarea.com";
...
export class LeadNurtureService {
  private readonly log = new Logger(LeadNurtureService.name);

  constructor(
    private readonly leads: LeadFunnelService,
  ...
  async runDaily(): Promise<void> {
    ...
    for (const tpl of TEMPLATES) {
      ...
      for (const l of candidates) {
        if (l.nurtureLog?.[tpl.key]) continue; // déjà envoyé
        if (!l.email) continue;
        try {
          const lang: Lang = (l.lang || "fr") as Lang;
          ...
          this.log.warn(
            `[Nurture] échec ${tpl.key} → ${l.email}: ${e?.message}`,
          );
        }
      }
    }
    if (sent > 0) this.log.log(`[Nurture] ${sent} emails envoyés`);
  }
  ...
  async runNow(): Promise<{ sent: number }> {
    ...
    for (const tpl of TEMPLATES) {
      ...
      for (const l of candidates) {
        if (l.nurtureLog?.[tpl.key]) continue;
        if (!l.email) continue;
        const lang: Lang = (l.lang || "fr") as Lang;
        await this.email.send({
          to: l.email,
