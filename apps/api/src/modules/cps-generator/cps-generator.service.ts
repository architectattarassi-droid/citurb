...
export type CpsGenerateInput = {
  ...
  /** Filigrane nominatif incrusté (réf. dossier · utilisateur · date) — traçabilité. */
  watermark?: string;
  /** Bloc de signatures à apposer en pied de document. */
  signatures?: Array<{ partie: string; role?: string; signataire?: string; signedAt?: string; status?: string }>;
};
...
export class CpsGeneratorService {
  ...
  private renderHtml(
    ...
    const wmSvg = wm
      ? "data:image/svg+xml," +
        encodeURIComponent(
          `<svg xmlns='http://www.w3.org/2000/svg' width='480' height='240'><text x='8' y='135' transform='rotate(-28 240 120)' fill='rgba(11,27,58,0.10)' font-size='17' font-weight='bold' font-family='Arial,Helvetica,sans-serif'>${wm}</text></svg>`,
        )
      : "";
...
<script>["copy","cut","contextmenu"].forEach(function(ev){document.addEventListener(ev,function(e){e.preventDefault();});});</script>
  ...
  private renderSignatures(
    signatures: CpsGenerateInput["signatures"],
    lang: CpsLang,
  ): string {
    ...
    const rows = signatures
      .map(
        (s) =>
          `<tr><td>${escapeHtml(s.partie || "—")}</td><td>${escapeHtml(s.role || "—")}</td><td>${escapeHtml(s.signataire || "")}</td><td>${escapeHtml(s.signedAt ? new Date(s.signedAt).toLocaleDateString("fr-FR") : "")}</td><td class="cit-status" style="color:${s.status === "SIGNE" ? "#166534" : "#92400e"}">${escapeHtml(s.status || "EN ATTENTE")}</td></tr>`,
      )
      .join("");
