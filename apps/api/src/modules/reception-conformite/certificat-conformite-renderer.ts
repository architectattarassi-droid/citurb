...
import { CertificatConformite, PermisHabiterState } from "./types";

type CertRendererOpts = {
  dossierTitle?: string;
  dossierRef?: string;
  commune?: string;
  permisConstruireRef?: string;
  permisConstruireDateDelivrance?: string;
  moaName?: string;
};
 ...
 */
@Injectable()
export class CertificatConformiteRenderer {
  renderCertificat(
    cert: CertificatConformite,
    permis: PermisHabiterState,
    opts: CertRendererOpts = {},
  ): string {
    ...
    const visitesRecap = (permis.visites || [])
      .map(
        (v) =>
          `<tr><td>${this.fmtDate(v.dateVisite)}</td><td>${this.escape(v.agentName)}</td><td>${this.escape(v.resultat)}</td><td>${this.escape(v.observations ?? "—")}</td></tr>`,
      )
      .join("");
