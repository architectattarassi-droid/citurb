 ...
 */
@Injectable()
export class PvReceptionRenderer {
  renderProvisoire(pv: ReceptionProvisoire, opts: RendererOpts = {}): string {
  ...
  private shell(args: {
    title: string;
    subtitle: string;
    docKind: "PROVISOIRE" | "DEFINITIVE" | "LEVEE" | "CERTIFICAT";
    docNumero: string;
    status: string;
...
<div class="footer">
  ...
  private gridIdent(opts: {
    ...
    date: string;
    montant?: number | null;
    extra?: Array<{ k: string; v: string }>;
  }): string {
    const cells: Array<{ k: string; v: string }> = [
      { k: opts.dateLabel, v: opts.date },
    ];
    if (opts.dossierTitle) cells.push({ k: "Dossier", v: opts.dossierTitle });
    if (opts.dossierRef) cells.push({ k: "Référence dossier", v: opts.dossierRef });
    if (opts.commune) cells.push({ k: "Commune", v: opts.commune });
    if (opts.permisConstruireRef) cells.push({ k: "Permis de construire", v: opts.permisConstruireRef });
    if (opts.montant != null) cells.push({ k: "Montant des travaux", v: `${this.fmtMontant(opts.montant)} MAD` });
  ...
  private tablePresents(arr: { nom: string; role: string; organisme?: string | null }[]): string {
  ...
  private tableChecklist(arr: { libelle: string; conforme: boolean; observation?: string | null }[]): string {
    if (!arr || arr.length === 0) return "<p>Aucun item.</p>";
    return `<table>
  ...
  private listReserves(arr: Reserve[], showLevee: boolean): string {
    if (!arr || arr.length === 0) return "<p>Aucune réserve.</p>";
    return arr.map((r) => {
  ...
  private galeriePhotos(arr: { url: string; legende?: string | null; piece?: string | null }[]): string {
    if (!arr || arr.length === 0) return "<p>Aucune photo.</p>";
  ...
  private signaturesBlock(arr: ReceptionSignature[]): string {
  ...
  private docKindLabel(k: string): string {
    return {
  ...
  private verifyUrl(hash: string | null | undefined): string | null {
    if (!hash) return null;
   ...
   */
  private qrSvgPlaceholder(payload: string): string {
    const size = 21;
    const cell = 5;
    let bits = "";
    ...
    for (let y = 0; y < size; y++) {
      for (let x = 0; x < size; x++) {
        ...
        let dark: boolean;
        if (isFinder) dark = onFinderBorder || finderCore;
        else dark = bits.charAt(y * size + x) === "1";
        if (dark) rects.push(`<rect x="${x * cell}" y="${y * cell}" width="${cell}" height="${cell}" />`);
      }
    }
    const dim = size * cell;
  ...
  private fmtMontant(n: number): string {
    try {
