...
import type {
  MreProfile, Procuration, EscrowAccount, EscrowMilestone,
  DiasporaDashboard, MissionType, MreCountry, ProcurationStatus,
} from "./mre-diaspora.types";
 ...
 */
@Injectable()
export class MreDiasporaService {
  private readonly logger = new Logger(MreDiasporaService.name);
  ...
  private async readPayload(dossierId: string): Promise<any> {
    const d = await this.prisma.dossier.findUnique({ where: { id: dossierId } });
    if (!d) throw new Error(`Dossier introuvable: ${dossierId}`);
    return { dossier: d, payload: (d.payload as any) || {} };
  }

  private async writePayload(dossierId: string, payload: any): Promise<void> {
    await this.prisma.dossier.update({ where: { id: dossierId }, data: { payload } });
  }

  /** Upgrade un utilisateur en profil MRE certifié. */
  async upgradeProfile(userId: string, input: {
  ...
  private profileCache = new Map<string, MreProfile>();

  getProfile(userId: string): MreProfile | null {
    return this.profileCache.get(userId) || null;
  }

  /** Crée une procuration eIDAS pour une mission. */
  async createProcuration(input: {
    dossierId: string; mreUserId: string; mandataireId: string; mandataireNom?: string;
    missionType: MissionType; scopes: string[]; dureeJours: number;
  }): Promise<Procuration> {
    const { payload } = await this.readPayload(input.dossierId);
    const procs: Procuration[] = payload.mreProcurations || [];

    const proc: Procuration = {
      id: randomUUID(),
  ...
  async signProcuration(dossierId: string, procId: string, signatureDataUrl: string, country: MreCountry): Promise<Procuration> {
    const { payload } = await this.readPayload(dossierId);
    const procs: Procuration[] = payload.mreProcurations || [];
    const proc = procs.find((p) => p.id === procId);
    if (!proc) throw new Error(`Procuration introuvable: ${procId}`);
    if (proc.status !== "PENDING_SIGNATURE") throw new Error(`Procuration non signable (statut ${proc.status})`);

    proc.signatureDataUrl = signatureDataUrl;
    proc.signedFromCountry = country;
    proc.signedAt = new Date().toISOString();
    proc.hashSha256 = createHash("sha256")
    ...
    }).catch((e: any) => this.logger.warn(`ProbativeLog MRE sign: ${e?.message}`));

    return proc;
  }

  /** Demande Apostille de La Haye (workflow placeholder). */
  async requestApostille(dossierId: string, procId: string): Promise<Procuration> {
    const { payload } = await this.readPayload(dossierId);
    const procs: Procuration[] = payload.mreProcurations || [];
    const proc = procs.find((p) => p.id === procId);
    if (!proc) throw new Error(`Procuration introuvable: ${procId}`);
    if (proc.status !== "SIGNED") throw new Error(`Procuration doit être signée avant Apostille`);
  ...
  async getEscrow(dossierId: string): Promise<EscrowAccount | null> {
    const { payload } = await this.readPayload(dossierId);
    return payload.mreEscrow || null;
  }

  /** Crée/initialise un compte escrow avec jalons. */
  async initEscrow(dossierId: string, mreUserId: string, milestones: { label: string; amountMad: number; conditionDescription: string }[]): Promise<EscrowAccount> {
    const { payload } = await this.readPayload(dossierId);
    const ms: EscrowMilestone[] = milestones.map((m) => ({
      id: randomUUID(), label: m.label, amountMad: m.amountMad,
      status: "PENDING" as const, conditionDescription: m.conditionDescription,
    }));
  ...
  async releaseMilestone(dossierId: string, milestoneId: string, preuveDocIds: string[]): Promise<EscrowAccount> {
    const { payload } = await this.readPayload(dossierId);
    const escrow: EscrowAccount = payload.mreEscrow;
    if (!escrow) throw new Error(`Pas de compte escrow pour ce dossier`);
    const m = escrow.milestones.find((x) => x.id === milestoneId);
    if (!m) throw new Error(`Jalon introuvable: ${milestoneId}`);
    if (m.status === "RELEASED") throw new Error(`Jalon déjà libéré`);
    ...
    }).catch((e: any) => this.logger.warn(`ProbativeLog MRE escrow: ${e?.message}`));

    return escrow;
  }

  /** Dashboard agrégé MRE. */
  async getDashboard(userId: string): Promise<DiasporaDashboard> {
    ...
    const procurations: Procuration[] = [];
    const escrowAccounts: EscrowAccount[] = [];
    const activeDossiers: { dossierId: string; title: string; phase: string }[] = [];

    for (const d of dossiers as any[]) {
      const pl = (d.payload as any) || {};
      if (Array.isArray(pl.mreProcurations)) procurations.push(...pl.mreProcurations);
      if (pl.mreEscrow) escrowAccounts.push(pl.mreEscrow);
      activeDossiers.push({
        dossierId: d.id,
  ...
  private renderProcurationHtml(input: { mreUserId: string; mandataireNom?: string; missionType: MissionType; scopes: string[]; dureeJours: number }): string {
