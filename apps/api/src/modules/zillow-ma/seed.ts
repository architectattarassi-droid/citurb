...
import type { ParcellePilote, ComparableTransaction, BienFamily, StatutJuridique } from "./types";

// ── PRNG déterministe ─────────────────────────────────────────────────────
function makePrng(seedInit: number) {
  let s = seedInit;
  return () => {
...
function makeRandInt(rand: () => number) {
  return (min: number, max: number) => Math.floor(rand() * (max - min + 1)) + min;
}
...
function pickWeighted<T extends { v: any; w: number }>(items: T[], rand: () => number): T["v"] {
...
// ── Zones pilotes : Casa-Anfa / Marrakech-Gueliz / Tanger-Centre ─────────
type Quartier = { name: string; dgi: number; weight: number };
type Zone = {
  villeId: "casablanca" | "marrakech" | "tanger";
  ville: string;
  bbox: { latMin: number; latMax: number; lngMin: number; lngMax: number };
  quartiers: Quartier[];
  marcheFactor: { min: number; max: number; central: number };
  n: number;
};

const ZONES: Record<"casablanca" | "marrakech" | "tanger", Zone> = {
  casablanca: {
    villeId: "casablanca",
    ville: "Casablanca",
...
const BIEN_FAMILY_DIST: Array<{ v: BienFamily; w: number }> = [
  { v: "terrain_nu",            w: 0.12 },
...
const STATUT_DIST: Array<{ v: StatutJuridique; w: number }> = [
...
const RUES: Record<string, string[]> = {
...
function pickQuartier(quartiers: Quartier[], rand: () => number): Quartier {
  const totalW = quartiers.reduce((s, q) => s + q.weight, 0);
  let r = rand() * totalW;
  for (const q of quartiers) { r -= q.weight; if (r <= 0) return q; }
  return quartiers[quartiers.length - 1];
}

function surfaceFor(bienFamily: BienFamily, randInt: (a: number, b: number) => number): number {
  switch (bienFamily) {
...
function genTitreFoncier(villeId: string, randInt: (a: number, b: number) => number): string {
  const prefix = villeId === "casablanca" ? "CA" : villeId === "marrakech" ? "MA" : "TG";
  return `${randInt(1000, 99999)}/${prefix}`;
}

function pickTags(rand: () => number, n = 2): string[] {
  const out: string[] = [];
...
export function generateParcelles(): ParcellePilote[] {
  const rand = makePrng(42);
  const randInt = makeRandInt(rand);
  const out: ParcellePilote[] = [];
  let pid = 0;
  for (const zoneKey of ["casablanca", "marrakech", "tanger"] as const) {
    ...
    for (let i = 0; i < z.n; i++) {
      ...
      let derniereTransaction: ParcellePilote["derniereTransaction"] = null;
      if (rand() < 0.60) {
...
export function generateComparables(): ComparableTransaction[] {
  ...
  const randInt = makeRandInt(rand);
  const gauss = makeGauss(rand);
  const out: ComparableTransaction[] = [];
  let cid = 0;
  for (const zoneKey of ["casablanca", "marrakech", "tanger"] as const) {
    const z = ZONES[zoneKey];
    ...
    for (let i = 0; i < nCity; i++) {
      ...
      const source: ComparableTransaction["source"] =
        sourceRoll < 0.40 ? "DGI_DECLARATIVE"
 ...
 */
export function quartierCentroids(parcelles: ParcellePilote[]): Map<string, { lat: number; lng: number }> {
  ...
  for (const [k, v] of groups) out.set(k, { lat: v.latSum / v.n, lng: v.lngSum / v.n });
  return out;
}
