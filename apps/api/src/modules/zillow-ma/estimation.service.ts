...
import type {
  BienFamily, ComparableTransaction, ConfidenceTier,
  EstimationInput, EstimationResult, ParcellePilote,
} from "./types";
...
function similarityBienFamily(a: BienFamily | undefined, b: BienFamily): number {
  if (!a) return 0.6;                    // pas de filtre → score moyen
  if (a === b) return 1.0;
  // Compatibilité résiduelle : terrain_nu ↔ terrain_constructible / villa ↔ appartement (cohabitation marché)
  const TERRAIN = new Set<BienFamily>(["terrain_nu", "terrain_constructible"]);
  const RESID = new Set<BienFamily>(["villa", "appartement", "immeuble"]);
  const TERTIAIRE = new Set<BienFamily>(["local_commercial", "bureau"]);
  if (TERRAIN.has(a) && TERRAIN.has(b)) return 0.55;
  if (RESID.has(a) && RESID.has(b)) return 0.45;
  if (TERTIAIRE.has(a) && TERTIAIRE.has(b)) return 0.50;
  return 0.20;
}

function tierFromConfidence(c: number): ConfidenceTier {
  if (c >= 80) return "HIGH";
...
export class EstimationService {
  ...
  estimate(
    ...
    return {
      ...
      methodologie:
        `Estimation pondérée à partir de ${weighted.length} comparables (rayon ${radius.toFixed(1)} km, ` +
      ...
      disclaimer:
        "Estimation algorithmique CITURBAREA — indication uniquement. Ne se substitue pas à " +
  ...
  private collectInRadius(
    ...
    return comparables.filter((c) => {
      ...
      if (m > maxMonths) return false;
      const d = distanceKm(input.lat, input.lng, c.lat, c.lng);
      return d <= radiusKm;
    });
  }

  private computeDgiReference(
    input: EstimationInput,
    parcelles: ParcellePilote[],
  ): { prixMin: number; prixMoy: number; prixMax: number } | null {
  ...
  private computeConfidence(args: {
