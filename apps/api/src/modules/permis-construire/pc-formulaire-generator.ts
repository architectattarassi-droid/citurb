...
import type { PcDraft, PcFormulaire } from "./types";

interface FormulaireSpec {
  code: PcFormulaire["code"];
  label: string;
  /** Renvoie HTML complet (avec <html>/<head>/<body>). */
  render: (draft: PcDraft, overrides?: Record<string, unknown>) => string;
  /** Pré-saisie automatique depuis le brouillon. */
  prefill: (draft: PcDraft) => Record<string, unknown>;
}
...
const DEMANDE_AUTORISATION: FormulaireSpec = {
...
const ENGAGEMENT_ARCHITECTE: FormulaireSpec = {
...
const NOTICE_INCENDIE: FormulaireSpec = {
...
const NOTICE_PMR: FormulaireSpec = {
  code: "NOTICE_PMR",
...
const NOTICE_THERMIQUE: FormulaireSpec = {
...
const REGISTRY: Record<PcFormulaire["code"], FormulaireSpec> = {
...
export function formulairesRequis(draft: PcDraft): PcFormulaire["code"][] {
  const base: PcFormulaire["code"][] = [
    "DEMANDE_AUTORISATION",
    "ENGAGEMENT_ARCHITECTE",
    "NOTICE_THERMIQUE",
  ];
  const t = draft.identification.projectType;
  if (t === "ERP" || t === "MOSQUEE") base.push("NOTICE_INCENDIE");
  if (t === "ERP" || t === "EQUIPEMENT_PUBLIC") base.push("NOTICE_PMR");
  return base;
}

/** Rend un formulaire en HTML imprimable. */
export function renderFormulaire(
  code: PcFormulaire["code"],
  draft: PcDraft,
  overrides?: Record<string, unknown>,
): { html: string; label: string } {
  const spec = REGISTRY[code];
  if (!spec) throw new Error(`Formulaire inconnu : ${code}`);
  return { html: spec.render(draft, overrides), label: spec.label };
}

/** Pré-saisie d'un formulaire (pour affichage UI avant validation). */
export function prefillFormulaire(
  code: PcFormulaire["code"],
  draft: PcDraft,
...
export function listFormulairesCatalog(): Array<{ code: PcFormulaire["code"]; label: string }> {
