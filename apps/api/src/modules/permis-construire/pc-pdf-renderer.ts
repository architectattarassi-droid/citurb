...
import type { PcDraft, PieceDefinition, PieceState } from "./types";

interface MasterRenderOptions {
  draft: PcDraft;
  checklist: Array<PieceDefinition & { state: PieceState }>;
  formulairesHtml: Array<{ code: string; label: string; html: string }>;
  /** Snapshot du contenu des pièces uploadées (chemin → base64) pour embed images. */
  embeds?: Record<string, { mimeType: string; base64: string }>;
}
 ...
 */
function computeHash(draft: PcDraft, checklist: MasterRenderOptions["checklist"]): string {
...
export function renderMaster(opts: MasterRenderOptions): MasterRenderResult {
...
function stripShell(full: string): string {
...
export function renderAttestation(opts: {
  draft: PcDraft;
  hash: string;
  qrPayload: string;
}): string {
