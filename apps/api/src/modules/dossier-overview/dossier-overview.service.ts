 ...
 */
@Injectable()
export class DossierOverviewService {
  private readonly logger = new Logger(DossierOverviewService.name);
  ...
  async getOverview(
    ...
    const payload: any =
      dossier.payload && typeof dossier.payload === "object" ? dossier.payload : {};
  ...
  private buildPhases(dossier: any, payload: any): PhaseSummary[] {
    // ── LEAD
    const lead = payload.lead ?? {};
    const leadDone =
      Boolean(dossier.mandatSigne) ||
      Boolean(payload.packValidation?.status) ||
      dossier.status !== "DRAFT";
    const leadSummary: PhaseSummary = {
      id: "lead",
    ...
    const packStatus: string | null = pv?.status ?? null;
    const packActivated = packStatus === "ACTIVATED";
    const manageStatus: PhaseStatus = packActivated
      ? "DONE"
      : leadDone
        ? "ACTIVE"
        : "PENDING";
    const manageSummary: PhaseSummary = {
      id: "manage",
      status: manageStatus,
    ...
    const permitSummary: PhaseSummary = {
      id: "permit",
    ...
    const tasks: any[] = Array.isArray(cal.tasks) ? cal.tasks : [];
    ...
    const pvList: any[] = Array.isArray(payload.pvChantier) ? payload.pvChantier : [];
    const lastVisit =
      pvList.length > 0
        ? pvList.reduce((acc, pv) => (pv.date && pv.date > acc ? pv.date : acc), "")
        : null;
    const photosRecentCount = pvList.reduce((acc, pv) => {
      const obs: any[] = Array.isArray(pv.observations) ? pv.observations : [];
    ...
    const deliverySummary: PhaseSummary = {
      id: "delivery",
  ...
  private detectCurrentPhase(phases: PhaseSummary[]): ParcoursPhaseId {
  ...
  private computeGlobalProgress(phases: PhaseSummary[], payload: any): number {
    // Chaque phase pèse 20%. Pour la phase courante, on regarde le %
    // chantier si disponible, sinon 50% par défaut.
    let pct = 0;
    for (const p of phases) {
      if (p.status === "DONE") pct += 20;
      else if (p.status === "ACTIVE") {
        if (p.id === "site") {
          const sitePct = Number((p.summary as any).progressPct) || 0;
  ...
  private buildPayments(dossier: any, payload: any): PaymentsBlock {
    const totalContractMAD = Number(dossier.packPriceMAD ?? payload.totalContractMAD ?? 0) || 0;
    const payments: any[] = Array.isArray(dossier.payments) ? dossier.payments : [];
    ...
    const rawSchedule: any[] = Array.isArray(payload.paymentSchedule)
      ? payload.paymentSchedule
      : [];
    const schedule: ScheduleItem[] = rawSchedule.map((s) => ({
  ...
  private buildDocuments(dossier: any): DocumentRef[] {
    const out: DocumentRef[] = [];
    const docs: any[] = Array.isArray(dossier.documents) ? dossier.documents : [];
    ...
    for (const k of keys) {
      const match = sorted.find((d) =>
        String(d.docType ?? "").toUpperCase().includes(k),
      );
      if (match) {
        out.push({
          id: match.id,
  ...
  private buildDocUrl(dossierId: string, docId: string): string {
  ...
  private buildExperts(dossier: any, payload: any): ExpertContact[] {
    const out: ExpertContact[] = [];
  ...
  private computeNextAction(
    ...
    if (!pv || pv.status === "PENDING_PAYMENT") {
      return {
        ...
        description:
          "Pour démarrer votre projet, finalisez le paiement de votre pack CITURBAREA.",
    ...
    if (pv.status === "PAYMENT_RECEIVED" || pv.status === "PENDING_ADMIN_VALIDATION") {
      return {
        ...
        description:
          "Notre équipe valide votre dossier sous 24 h. Vous recevrez un email dès activation.",
    ...
    if (!dossier.mandatSigne) {
      return {
        ...
        description:
          "Le contrat d'architecte est requis pour engager les démarches Rokhas.",
    ...
    if (nextSchedule) {
      const due = new Date(nextSchedule.dueDate as string).getTime();
      const days = Math.round((due - Date.now()) / 86_400_000);
      const severity: "urgent" | "warning" | "info" =
        days <= 3 ? "urgent" : days <= 14 ? "warning" : "info";
      return {
    ...
    const pvList: any[] = Array.isArray(payload.pvChantier) ? payload.pvChantier : [];
    const pvUnsigned = pvList.find(
      (p) => p.status === "SIGNED_PARTIEL" || p.status === "DRAFT",
    );
    if (pvUnsigned) {
      return {
  ...
  private countReservesFromPhases(rok: any): number {
    if (!rok) return 0;
    const phases: any[] = Array.isArray(rok.phases) ? rok.phases : [];
    return phases.filter((p) => p.statut === "RESERVES" || p.motifRejet).length;
  }

  private averageProgress(tasks: any[]): number {
