...
import { BAREME_CNOA_2021, type CategoryCode, type P2Section } from "../../tome-2/p2/pricing.service";

export type P3QuoteInput = {
  // Section et catégorie de bâtiment (mêmes que P2)
  section: P2Section;
...
@Injectable()
export class P3PricingService {
  computeQuote(input: P3QuoteInput) {
    const cat = BAREME_CNOA_2021[input.categoryCode];
    if (!cat) throw new Error(`Catégorie inconnue : ${input.categoryCode}`);
