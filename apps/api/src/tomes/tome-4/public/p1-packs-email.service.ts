 ...
 */
export class P1PacksEmailService {
  private readonly ttlMs = 15 * 60 * 1000; // 15 minutes
  private readonly store = new Map<string, CodeEntry>();

  async requestCode(key: string, email: string, orderSnapshot: any) {
    ...
    if (smtpHost && smtpPort && smtpUser && smtpPass) {
      const secure =
        String(process.env.SMTP_SECURE || "") === "true" ? true : smtpPort === 465;
  ...
  verifyCode(key: string, code: string) {
  ...
  private buildText(code: string, order: any) {
    const o = order || {};
    ...
    if (hasNew) {
      lines.push(
        ...
        `- Niveau: ${proj.constructionLevel || "—"}`,
        `- Pack: ${pricing.packLabel || pricing.pack || "—"}`,
        "",
        "Services sélectionnés :",
        `- BET: ${services.betMode === "EXTERNAL" ? "Votre BET" : "BET via plateforme"}`,
        `- MOD (validation paiements): ${services.modEnabled ? "Oui" : "Non"}`,
        `- Décoration intérieure: ${services.decoEnabled ? "Oui" : "Non"}`,
        `- Mandat entreprise: ${services.mandateEntreprise ? "Oui" : "Non"}`,
        services.addRemoteFollow ? `- Suivi à distance: Oui` : `- Suivi à distance: Non`,
        "",
        "Montants indicatifs (MAD HT) :",
        `- Pack: ${fmt(pricing.packMAD)} MAD`,
        `- Suivi à distance: ${fmt(pricing.remoteFollowMAD)} MAD`,
        `- BET: ${fmt(pricing.betMAD)} MAD`,
        `- MOD: ${fmt(pricing.modMAD)} MAD`,
        `- Décoration: ${fmt(pricing.decoMAD)} MAD`,
        `- TOTAL: ${fmt(pricing.totalMADRounded ?? pricing.totalMAD)} MAD`,
        "",
        "NB: Les mécanismes internes de calcul ne sont pas affichés. Seuls les montants sont communiqués.",
        "",
