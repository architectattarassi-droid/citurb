...
/**
 * QuoteInvoiceService — génère HTML imprimable de DEVIS et FACTURES depuis
 * un dossier complet (toutes portes P1-P6).
 *
 * Source de vérité :
 *   - Coordonnées client : Dossier.clientNom / raisonSociale / RC / ICE / etc.
 *   - Lignes de prestation : Dossier.payload.brief.quoteSnapshot
 *   - Numérotation : Dossier.payload.quoteRef / invoiceRef (auto-généré)
 *   - Paramètres atelier : env (ATELIER_NOM, ATELIER_ICE, ...) + override admin
 *
 * Émet du HTML A4 prêt à imprimer (window.print) avec watermark "DEVIS"
 * (en filigrane discret) ou tampon "FACTURE PAYÉE / IMPAYÉE".
 *
 * TVA : 20% par défaut (Maroc). Configurable via env CIT_TVA_PERCENT.
 */

export type DocumentType = "QUOTE" | "INVOICE";

export type AtelierIdentity = {
  nom?: string;
...
export type DocumentParams = {
  ...
  emisLe?: Date;
  echeanceLe?: Date;
  paiementStatut?: "PAYEE" | "IMPAYEE" | "PARTIELLE";
...
export class QuoteInvoiceService {
  ...
  async getOrAssignNumero(dossierId: string, type: DocumentType): Promise<string> {
    ...
    for (const d of allDossiers) {
      const ref = (d.payload as any)?.[key];
  ...
  async renderHtml(dossierId: string, params: DocumentParams): Promise<string> {
    const dossier = await this.prisma.dossier.findUniqueOrThrow({
      where: { id: dossierId },
    });
    ...
    const payload: any = dossier.payload || {};
    const brief: any = payload.brief || {};
    const quote: any = brief.quoteSnapshot || payload.quoteSnapshot || {};
    ...
    if (typeof quote?.honoraires?.totalHT === "number") totalHT = quote.honoraires.totalHT;
    else if (typeof quote?.amounts?.totalHT === "number") totalHT = quote.amounts.totalHT;
  ...
  async createDevisLibre(
    emetteurId: string,
    input: { titre: string; lignes: any[]; clientInfo?: any; tva?: number; conditions?: string; porteType?: string },
  ) {
  ...
  private buildLignes(porteType: string, brief: any, quote: any): { designation: string; quantite: number; unite: string; prixUnitaire: number; totalHT: number }[] {
  ...
  private defaultEcheance(emisLe: Date, type: DocumentType): Date {
  ...
  private shell(d: {
    ...
    atelier: AtelierIdentity;
    dossier: any;
    lignes: { designation: string; quantite: number; unite: string; prixUnitaire: number; totalHT: number }[];
    totalHT: number;
    tva: number;
    tvaPct: number;
    totalTTC: number;
    ...
    const fmtDate = (date: Date) => date.toLocaleDateString("fr-MA", { day: "2-digit", month: "long", year: "numeric" });
    const fmtMAD = (n: number) => n.toLocaleString("fr-MA") + " DH";

    const clientLabel = d.dossier.raisonSociale || d.dossier.clientNom || "Client";
    const clientLines: string[] = [];
    if (d.dossier.raisonSociale) clientLines.push(`<strong>${esc(d.dossier.raisonSociale)}</strong>`);
    if (d.dossier.clientNom)     clientLines.push(esc(d.dossier.clientNom));
    if (d.dossier.representant)  clientLines.push(`Représentant : ${esc(d.dossier.representant)}`);
    if (d.dossier.address || d.dossier.commune) clientLines.push(esc([d.dossier.address, d.dossier.commune].filter(Boolean).join(" — ")));
    if (d.dossier.ice) clientLines.push(`ICE : ${esc(d.dossier.ice)}`);
    if (d.dossier.rc)  clientLines.push(`RC : ${esc(d.dossier.rc)}`);
    if (d.dossier.clientTel)   clientLines.push(`Tél : ${esc(d.dossier.clientTel)}`);
    if (d.dossier.clientEmail) clientLines.push(`Email : ${esc(d.dossier.clientEmail)}`);
