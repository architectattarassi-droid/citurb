...
/**
 * UniversalContractService — génère HTML imprimable des contrats P3/P4/P5/P6.
 *
 * Le P2 (contrat type unifié Construction CNOA) reste géré par
 * P2ContractService dans tome-2. Ce service-ci complète la couverture
 * pour les 4 autres portes:
 *
 *  - P3: Contrat de Maîtrise d'Ouvrage Déléguée (10% du coût de réalisation,
 *        escrow obligatoire, coordination multi-corps de métier)
 *  - P4: Contrat de prestation Analyse foncière (1% prix vente, watermark,
 *        paywall téléchargement)
 *  - P5: Contrat de prestation Expertise (forfait selon 4 types rapports)
 *  - P6: Contrat-cadre Prestataire / Fournisseur (engagement plateforme +
 *        escrow paiements pour prestataires, catalogue dynamique pour
 *        fournisseurs)
 *
 * Variables remplies depuis Dossier (clientNom, raisonSociale, brief, quote)
 * + AdminParams (numéro contrat, signataire CITURBAREA, délais, pénalités).
 */

export type UniversalAdminParams = {
  contratNumero?: string;
  archNom?: string; // Représentant CITURBAREA signataire
  archICE?: string;
  archRC?: string;
  archDomicile?: string;
  archTel?: string;
...
export type UniversalDossierData = {
  id: string;
  porteType: string;
  title?: string;
  commune?: string;
...
@Injectable()
export class UniversalContractService {
  /**
   * Dispatcher: choisit le template selon porteType.
   */
  renderHtml(d: UniversalDossierData, admin: UniversalAdminParams = {}): string {
  ...
  // ── P3 — Contrat MOD ──────────────────────────────────────────────
  private renderP3(d: UniversalDossierData, admin: UniversalAdminParams): string {
    const brief = d.brief || {};
    const quote = brief.quoteSnapshot || {};
    const corpsList: string[] = brief.corpsMetiers || [];
  ...
  private renderP4(d: UniversalDossierData, admin: UniversalAdminParams): string {
    const brief = d.brief || {};
    const quote = brief.quoteSnapshot || {};
    const meta = quote.meta || {};
    ...
    return this.shell({
      ...
      bodyHtml: `
        ...
        ${quote.deliverables?.length ? `
          <ul>${quote.deliverables.map((dl: string) => `<li>${dl}</li>`).join("")}</ul>
  ...
  private renderP5(d: UniversalDossierData, admin: UniversalAdminParams): string {
    const brief = d.brief || {};
    const quote = brief.quoteSnapshot || {};
    const meta = quote.meta || {};
    const amounts = quote.amounts || {};
  ...
  private renderP6(d: UniversalDossierData, admin: UniversalAdminParams): string {
    const brief = d.brief || {};
  ...
  private renderGeneric(d: UniversalDossierData, admin: UniversalAdminParams): string {
    return this.shell({
  ...
  private shell(opts: {
    ...
    contratNumero?: string;
    bodyHtml: string;
    d: UniversalDossierData;
    admin: UniversalAdminParams;
  }): string {
...
<div class="footer">
  ...
  // ── Helpers ───────────────────────────────────────────────────────
  private dash = (s: any) => (s == null || s === "" ? "_______________" : String(s));
  private fmt = (n: any) => Number.isFinite(Number(n))
    ? new Intl.NumberFormat("fr-MA", { maximumFractionDigits: 0 }).format(Number(n))
    : "—";

  private metaTable(rows: Array<[string, any]>): string {
  ...
  private honoTable(honos: any): string {
    if (!honos || !honos.totalTTC) return "";
