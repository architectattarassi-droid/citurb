...
/**
 * ReportRendererService — génère HTML imprimable pour rapports P4 / P5
 *
 * Doctrine T7-R-EXPORT-001:
 *  - Tout rapport téléchargé doit être watermarqué
 *  - Téléchargement autorisé seulement si packValidation.status === ACTIVATED
 *  - Watermark: "Rapport exclusif CITURBAREA — utilisable sur autorisation écrite"
 *
 * Le rendu reprend la pattern HTML imprimable (cf. contract.service.ts):
 *  l'utilisateur final clique Imprimer → Save as PDF.
 *  Le watermark est intégré au HTML via CSS @media print + position fixed
 *  rotated avec opacity → présent à l'export PDF.
 */
...
export type ReportContext = {
  ...
  // Optionnel: rendu admin preview (bypass gating)
  isAdminPreview?: boolean;
  // Optionnel: contenu spécifique fourni par admin (texte rapport, conclusion, etc.)
  adminContent?: {
    sections?: { title: string; body: string }[];
    conclusion?: string;
    expertNom?: string;
    expertNumeroOrdre?: string;
    dateDelivrance?: string; // ISO
  };
};
...
export class ReportRendererService {
  renderHtml(ctx: ReportContext): string {
...
<div class="footer">
  ...
  private defaultSections(ctx: ReportContext): { title: string; body: string }[] {
    const isP4 = ctx.porteType === "P4";
    if (isP4) {
  ...
  private defaultConclusion(_ctx: ReportContext): string {
