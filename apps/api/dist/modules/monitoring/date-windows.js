 ...
 */
function casablancaDayWindow(ref, dayOffset = 0) {
    ...
    const offset = tzOffsetMs(new Date(wallMidnight), analytics_service_1.CASABLANCA_TZ);
    const startMs = wallMidnight - offset;
    const endMs = startMs + 24 * 60 * 60 * 1000 - 1;
    return { startMs, endMs };
}
/** Fenêtre de N jours pleins se terminant la veille (incluse) de `ref`. */
function lastNDaysWindow(ref, n) {
 ...
 */
function parseBoundMs(input, endOfDay = false) {
    if (!input)
        return null;
    const trimmed = input.trim();
    if (/^\d{13}$/.test(trimmed))
        return Number(trimmed); // epoch ms
