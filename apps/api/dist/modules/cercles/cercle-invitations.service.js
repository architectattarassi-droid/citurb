...
exports.CercleInvitationsService = void 0;
...
let CercleInvitationsService = class CercleInvitationsService {
    ...
    async inviteByEmail(cercleId, fromUserId, emails, message) {
        ...
        if (!emails || emails.length === 0)
            throw new common_1.BadRequestException("Aucun email");
        ...
        if (cleaned.length === 0)
            throw new common_1.BadRequestException("Aucun email valide");
        const results = [];
    ...
    async lookupToken(token) {
        ...
        if (inv.usedAt)
            throw new common_1.BadRequestException("Invitation déjà utilisée");
    ...
    async signupViaInvite(token, input) {
        ...
        if (!input.displayName?.trim())
            throw new common_1.BadRequestException("Nom d'affichage requis");
        const inv = await this.prisma.cercleInvitation.findUnique({
            where: { token },
    ...
    async listForCercle(cercleId, viewerId) {
...
exports.CercleInvitationsService = CercleInvitationsService;
