...
const dossier_shape_1 = require("./dossier-shape");
const phase_mapping_1 = require("./phase-mapping");
...
let PhaseEngineService = PhaseEngineService_1 = class PhaseEngineService {
    ...
    async getV7Index(dossierId) {
        ...
        const shape = (0, dossier_shape_1.dossierToShape)(dossier);
        const localValid = (0, dossier_shape_1.validateDossierShape)(shape);
        if (!localValid.ok) {
            this.logger.warn(`[v7] dossier ${dossierId} shape invalide (vague 1, non bloquant) : ${localValid.errors.join(', ')}`);
        }
        const index = catalog.buildPhaseIndex(shape);
        this.v7IndexCache.set(dossierId, index);
        return index;
    }
    invalidateV7Cache(dossierId) {
    ...
    async advancePhase(dossierId, fromPhase, toPhase, acteurId, note) {
        if (this.useV7) {
            try {
                ...
                this.logger.error(`[v7] advancePhase ${dossierId} ${fromPhase}→${toPhase} erreur inattendue (${e?.message}) — fallback legacy`);
            }
        }
     ...
     */
    async v7AdvancePhase(dossierId, fromPhase, toPhase, note) {
        const fromV7 = (0, phase_mapping_1.toV7)(fromPhase);
        const toV7Code = (0, phase_mapping_1.toV7)(toPhase);
        ...
        if (!targetNode) {
            ...
            throw new common_1.BadRequestException(`Phase cible "${toV7Code}" introuvable dans le DAG v7 du dossier ${dossierId}`);
        }
        if (!targetNode.prerequis.includes(fromV7)) {
            throw new common_1.BadRequestException(`Transition invalide ${fromV7} → ${toV7Code} dans le DAG v7 du dossier (prerequis de la cible : ${targetNode.prerequis.join(', ') || 'aucun'})`);
        }
    ...
    async initBrief(dossierId) {
        const existing = await this.prisma.dossierPhaseRecord.findFirst({
            where: { dossierId, phase: { in: (0, phase_mapping_1.phaseVariants)('00_BRIEF') } },
        });
        if (existing)
            return existing;
    ...
    async advanceBriefToEsquisse(dossierId, author) {
        if (this.useV7) {
            try {
                ...
                if (result !== undefined)
                    return result; // null si déjà avancé / dossier inconnu
            }
            catch (e) {
                this.logger.error(`[v7] advanceBriefToEsquisse failed for ${dossierId} (${e?.message}) — fallback legacy`);
                // tomber sur le chemin legacy ci-dessous
            }
        }
        return this.legacyAdvanceBriefToEsquisse(dossierId, author);
    }
    async legacyAdvanceBriefToEsquisse(dossierId, author) {
        const dossier = await this.prisma.dossier.findUnique({
            where: { id: dossierId },
            select: { phase: true },
        });
        ...
        const currentV7 = (0, phase_mapping_1.toV7)(dossier.phase ?? exports.PHASE_BRIEF);
        if (currentV7 !== '00_BRIEF')
     ...
     * de graphe.
     */
    async validatePhase(dossierId, fromPhase, acteurId, note) {
        if (this.useV7) {
            try {
                ...
                this.logger.error(`[v7] validatePhase failed for ${dossierId} ${fromPhase} (${e?.message}) — fallback legacy`);
            }
        }
     ...
     */
    async v7ValidatePhase(dossierId, fromPhase, note) {
        const fromV7 = (0, phase_mapping_1.toV7)(fromPhase);
        ...
        if (successors.length === 0) {
            await this.prisma.dossierPhaseRecord.updateMany({
                where: { dossierId, phase: { in: (0, phase_mapping_1.phaseVariants)(fromPhase) } },
        ...
        if (successors.length > 1) {
            this.logger.warn(`[v7] ${dossierId} : ${successors.length} successeurs de ${fromV7} (${successors.map(s => s.code).join(', ')}) — choix déterministe du premier en topo-order (UX explicite = vague future)`);
        }
     ...
     */
    async v7AdvanceFromBrief(dossierId, author) {
        const dossier = await this.prisma.dossier.findUnique({
            where: { id: dossierId },
            select: { phase: true },
        });
        if (!dossier)
            return null; // dossier inexistant → no-op (pas de fallback à tenter)
        const currentV7 = (0, phase_mapping_1.toV7)(dossier.phase ?? exports.PHASE_BRIEF);
        if (currentV7 !== '00_BRIEF')
            return null; // déjà avancé
        ...
        if (successors.length > 1) {
            ...
            this.logger.error(`[v7] dossier ${dossierId} : ${successors.length} successeurs de 00_BRIEF (${successors.map(s => s.code).join(', ')}) — choix déterministe du premier en topo-order`);
        }
        const toPhase = successors[0].code;
        // Vague B.3 : écriture centralisée via v7WritePhaseTransition.
    ...
    async getPhaseStatus(dossierId) {
        if (this.useV7) {
            try {
                ...
                this.logger.error(`[v7] getPhaseStatus ${dossierId} a échoué (${e?.message}) — fallback legacy`);
            }
        }
    ...
    async legacyGetPhaseStatus(dossierId) {
     ...
     */
    async v7AssertAdvanceLegit(dossierId, fromPhase, toPhase) {
        const index = await this.getV7Index(dossierId);
        ...
        if (!target.prerequis.includes(fromPhase)) {
            this.logger.warn(`[v7] désalignement legacy/v7 : ${fromPhase} → ${toPhase} accepté par legacy mais "${fromPhase}" n'est pas dans prerequis de "${toPhase}" (${target.prerequis.join(', ')})`);
        }
    }
    /**
     ...
     */
    async v7GetPhaseStatus(dossierId) {
        const index = await this.getV7Index(dossierId);
        const records = await this.prisma.dossierPhaseRecord.findMany({
            where: { dossierId },
            orderBy: { createdAt: 'asc' },
        ...
        if (status.currentsAll.length > 1) {
            this.logger.warn(`[v7] dossier ${dossierId} a ${status.currentsAll.length} phases EN_COURS en parallèle : ${status.currentsAll.join(', ')} (signature legacy retourne le 1er)`);
        }
        return {
     ...
     */
    async v7WritePhaseTransition(dossierId, fromPhase, toPhase, note) {
        const now = new Date();
        await this.prisma.dossierPhaseRecord.updateMany({
            where: { dossierId, phase: { in: (0, phase_mapping_1.phaseVariants)(fromPhase) } },
            data: { statut: 'COMPLETE', dateFin: now },
    ...
    computeV7Status(index, records) {
        ...
        for (const r of records) {
            const v7Code = (0, phase_mapping_1.toV7)(r.phase);
        ...
        for (const code of nodeCodes) {
            ...
            if (rec?.statut === 'COMPLETE')
                completedIds.push(code);
            else if (rec?.statut === 'EN_COURS')
                currentsAll.push(code);
            else
                pendingIds.push(code);
        }
        return {
            completedIds,
            currentsAll,
     ...
     */
    async getDossierPhasesSnapshot(dossierId) {
        const catalog = this.tryLoadCatalog();
        if (!catalog) {
            throw new common_1.BadRequestException('[snapshot] catalogue v7 indisponible — vérifier le build de @citurbarea/contracts');
        }
        const dossier = await this.prisma.dossier.findUnique({
            where: { id: dossierId },
            select: {
        ...
        if (!dossier) {
            ...
            const err = new Error(`dossier ${dossierId} introuvable`);
            err.status = 404;
            throw err;
        }
        const shape = (0, dossier_shape_1.dossierToShape)(dossier);
        ...
        if (catalog.validateDossier) {
            const v7 = catalog.validateDossier({ ...shape, id: dossier.id });
            if (!v7.ok) {
                this.logger.warn(`[snapshot] dossier ${dossierId} non conforme v7 : ${v7.errors.join(' | ')}`);
                const err = new Error(`Dossier ${dossierId} non conforme au catalogue v7`);
                err.status = 422;
                err.details = { errors: v7.errors };
                throw err;
            }
        }
        const index = catalog.buildPhaseIndex({ ...shape, id: dossier.id });
        const records = await this.prisma.dossierPhaseRecord.findMany({
            where: { dossierId },
            orderBy: { createdAt: 'asc' },
        ...
        for (const r of records) {
            ...
            datesByCode.set((0, phase_mapping_1.toV7)(r.phase), { dateDebut: r.dateDebut ?? null, dateFin: r.dateFin ?? null });
        }
        const nodes = index.ordered.map((n) => {
            const rec = status.statusByCode.get(n.code);
            const dates = datesByCode.get(n.code);
            const nodeStatus = rec?.statut === 'COMPLETE' ? 'DONE' : rec?.statut === 'EN_COURS' ? 'IN_PROGRESS' : 'NOT_STARTED';
            return {
                code: n.code,
                slug: n.slug,
                title: n.title,
