...
let SosService = class SosService {
    ...
    email;
    incidents;
    log = new common_1.Logger("SosService");
    constructor(twilio, email, incidents) {
        this.twilio = twilio;
        this.email = email;
     ...
     */
    async trigger(incidentId, contacts = {}, actor = {}) {
        ...
        const emergencyContacts = await this.incidents.findContactsForType(incident.type);
        const geolocText = incident.geoloc
            ? `https://maps.google.com/?q=${incident.geoloc.lat},${incident.geoloc.lng}`
            : "(géoloc non disponible)";
        const baseBody = `[SOS CITURBAREA] ${incident.numero} — ` +
        ...
        for (const t of smsTargets) {
            ...
            if (!r.ok)
                errors.push(`sms_${t.who}:${r.error || "fail"}`);
        }
        ...
        for (const t of waTargets) {
            ...
            if (!r.ok)
                errors.push(`wa_${t.who}:${r.error || "fail"}`);
        }
        ...
        for (const t of emailTargets) {
            ...
            if (!r.ok)
                errors.push(`email_${t.who}:${r.error || "fail"}`);
        }
        ...
        if (incident.severite === "CRITICAL") {
            for (const ec of emergencyContacts) {
                if (ec.email) {
                    ...
                    if (!r.ok)
                        errors.push(`emergency_${ec.code}:${r.error || "fail"}`);
                }
                emergencyCodes.push(ec.code);
            }
        }
        // ── 5) Persistance + ProbativeLog (via incidents-chantier.service) ─
        const updated = await this.incidents.markSosTriggered(incidentId, notified, emergencyCodes);
        return {
    ...
    // ── Helpers ────────────────────────────────────────────────────
    buildSosEmailHtml(incident, contacts, geolocText, typeLabel) {
        const photosHtml = incident.photos && incident.photos.length
            ? `<p><b>Photos :</b><br>${incident.photos
                .slice(0, 6)
                .map((p) => `<img src="${this.escapeAttr(p.url)}" style="max-width:280px;margin:6px;border:1px solid #ddd;border-radius:6px" />`)
                .join("")}</p>`
            : "";
        return `
...
exports.SosService = SosService = __decorate([
    ...
    __metadata("design:paramtypes", [twilio_service_1.TwilioService,
        ...
        incidents_chantier_service_1.IncidentsChantierService])
], SosService);
