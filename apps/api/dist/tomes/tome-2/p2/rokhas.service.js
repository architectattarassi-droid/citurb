...
exports.RokhasService = exports.PHASES_ARCHITECTE_ONLY = exports.ROKHAS_PHASES = void 0;
const common_1 = require("@nestjs/common");
const tome_at_1 = require("../../tome-at");
exports.ROKHAS_PHASES = {
...
exports.PHASES_ARCHITECTE_ONLY = [1, 2, 3, 4, 5, 6, 8, 10];
// Phase 7 (complément) et 9 (paiement taxes) peuvent être déléguées
let RokhasService = class RokhasService {
    prisma;
    constructor(prisma) {
        this.prisma = prisma;
    }
    async getOrCreate(dossierId) {
        ...
        return this.prisma.rokhasDossier.create({
            data: {
                dossierId,
                phases: {
                    create: Object.entries(exports.ROKHAS_PHASES).map(([phase, info]) => ({
                        phase: Number(phase),
    ...
    async syncFromRokhas(dossierId, data) {
        ...
        await Promise.all(Array.from({ length: 10 }, (_, i) => i + 1).map(p => {
            let statut;
            if (p < phaseActuelle)
                statut = 'TERMINE';
            else if (p === phaseActuelle)
                statut = isTermine ? 'TERMINE' : 'EN_COURS';
            else
                statut = 'EN_ATTENTE';
    ...
    async advancePhase(dossierId, toPhase, actorId, remarques) {
        const rd = await this.getOrCreate(dossierId);
        const now = new Date();
        const currentPhase = rd.phases.find((p) => p.phase === rd.phaseActuelle);
    ...
    async addDocument(dossierId, doc) {
        const rd = await this.getOrCreate(dossierId);
    ...
    async validateDocument(docId, userId) {
    ...
    async sendDocumentToClient(docId) {
        return this.prisma.rokhasDocument.update({
            where: { id: docId },
    ...
    async getPhaseTimeline(dossierId) {
        const rd = await this.getOrCreate(dossierId);
        return {
            rokhasDossier: rd,
            phases: rd.phases,
    ...
    async getClientView(dossierId) {
        ...
        return {
            ...
            documents: rd.documents.filter((d) => d.visibleClient).map((d) => ({
                id: d.id,
                nom: d.nom,
                type: d.type,
                statut: d.statut,
    ...
    static PHASES_CLIENT = [7, 9]; // Complément + Paiement taxes
    // Permis d'habiter = dossier séparé géré aussi par le client
    static PHASE_PERMIS_HABITER = 'habiter';
    async submitClientData(dossierId, data) {
        const rd = await this.getOrCreate(dossierId);
        // Merge avec les données existantes par phase
        const existing = rd.clientData || {};
        const merged = {
            ...existing,
            [`phase_${data.phase}`]: {
    ...
    async getClientData(dossierId) {
        const rd = await this.getOrCreate(dossierId);
        return {
            clientData: rd.clientData || {},
            pendingRokhasSync: rd.pendingRokhasSync || false,
            phaseActuelle: rd.phaseActuelle,
            statut: rd.statut,
    ...
    async saveSaisieData(dossierId, fields, actorId) {
        const rd = await this.getOrCreate(dossierId);
    ...
    async signEsign(dossierId, actorId) {
        const rd = await this.getOrCreate(dossierId);
        if (rd.phaseActuelle >= 4) {
    ...
    async markSyncComplete(rokhasDossierId) {
        await this.prisma.rokhasDossier.update({
...
exports.RokhasService = RokhasService;
exports.RokhasService = RokhasService = __decorate([
