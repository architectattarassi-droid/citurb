...
exports.P2ContractService = void 0;
const common_1 = require("@nestjs/common");
let P2ContractService = class P2ContractService {
    renderContractHtml(d, admin = {}) {
        ...
        const dashShort = (s) => (s == null || s === "" ? "……………………" : String(s));
        const dashPct = (n) => (n == null || n === "" ? "……" : `${Number(n).toFixed(2)}`);
        ...
        const r = (v, def) => v != null ? `${Number(v).toFixed(2)}` : def;
        const cumul = (vals) => {
            let s = 0;
            const out = [];
            for (const v of vals) {
...
exports.P2ContractService = P2ContractService;
exports.P2ContractService = P2ContractService = __decorate([
...
function numberToFrenchWords(n) {
    ...
    const rest = n % 1000;
    if (millions > 0)
        parts.push(`${millions === 1 ? "un" : under1000(millions)} million${millions > 1 ? "s" : ""}`);
    if (thousands > 0)
        parts.push(`${thousands === 1 ? "" : under1000(thousands) + " "}mille`.trim());
    if (rest > 0)
