...
let DossierService = class DossierService {
     ...
     */
    async assignIntervenant(dossierId, input) {
        if (!input?.userId)
        ...
        if (existing)
            throw new common_1.BadRequestException('Ce pro est déjà affecté à ce dossier');
        const pro = user.proProfile;
        const metier = (input.metier ?? pro?.metier ?? null);
    ...
    async listIntervenants(dossierId) {
