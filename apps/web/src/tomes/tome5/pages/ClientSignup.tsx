 ...
 *
 * Inscription dédiée aux particuliers / clients (projets de construction,
 * rénovation, foncier, expertises). Recueille les données qui constituent un
 * lead — identité + contact — crée un compte CLIENT puis ouvre la session.
 *
...
export default function ClientSignup() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (phone.replace(/[^0-9+]/g, "").length < 8) {
      setError("Numéro de téléphone invalide (format conseillé : +212…).");
      return;
    }
    ...
    try {
      ...
      // Ouvre la session proprement via le contexte d'authentification.
...
const S: Record<string, React.CSSProperties> = {
  ...
  sub: { fontSize: 14.5, color: "rgba(11,27,58,0.68)", margin: 0 },
  error: {
    padding: "14px 16px",
